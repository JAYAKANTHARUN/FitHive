...
const Checkout = () => {
    ...
    const handlePaymentSuccess = async (response) => {
        ...
        try {
            await fetch('https://fithive-e-commerce.onrender.com/api/send-email', {
                method: 'POST',
                headers: {
    ...
    const fetchKeyId = async () => {
        try {
            const response = await fetch('https://fithive-e-commerce.onrender.com/api/keyid',{
                headers: {
                    'authorization': JSON.parse(localStorage.getItem('token'))
                }
            });
    ...
    const handleplaceorderandpay = async (e) => {
        if (ordername && ordermobilenumber && orderaddress) {
            if (paymentmethod === 'online') {
                ...
                let order = await fetch(`https://fithive-e-commerce.onrender.com/checkout/${auth._id}`, {
                    method: 'post',
                ...
                const options = {
                    ...
                    "handler": async function (response) {
                        ...
                        let order = await fetch(`https://fithive-e-commerce.onrender.com/addorder/${auth._id}`, {
                            method: 'post',
                            body: JSON.stringify({ userid: auth._id, ordername: ordername, ordermobilenumber: ordermobilenumber, orderaddress: orderaddress, paymentmethod: paymentmethod, totalamount: totalamount, ordertime: new Date(), orderproducts: cart }),
                            headers: {
                                "Content-Type": "application/json",
                        ...
                        navigate('/orders')
                        let clear = await fetch(`https://fithive-e-commerce.onrender.com/clearcart/${auth._id}`, {
                            method: 'delete',
                            headers: {
                                'authorization': JSON.parse(localStorage.getItem('token'))
                            }
