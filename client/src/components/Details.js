...
const Details = () => {
    ...
    const getdetails = async () => {
        let result = await fetch(`https://fithive-e-commerce.onrender.com/details/${params.id}`)
    ...
    const addtocart = async (id, name, price, company, category, image, quantity) => {
        if (auth) {
            setIsLoading(true)
            let userid = auth._id
            let result = await fetch(`https://fithive-e-commerce.onrender.com/userproducts/${id}`, {
                method: 'post',
